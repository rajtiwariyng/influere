...
import "./Wallet.css";
import "./ConsultancyCategoryShortlisted.css";
...
const ConsultancyCategoryShortlisted = ({ category }) => {
  ...
  const [showAuctionInfluencerModal, setShowAuctionInfluencerModal] =
    useState(false);
  const [showAuctionCollaboratorModal, setShowAuctionCollaboratorModal] =
    useState(false);
  ...
  const outgoingWorkOrdersData =
    outgoingWorkOrders.length > 0 ? outgoingWorkOrders : [];

  // Get unique work order numbers for the dropdown
  const outgoingOrderNumbers = [
    ...new Set(outgoingWorkOrdersData.map((wo) => wo.orderNumber)),
  ];

  // Filter outgoing work orders by selected order number
  const filteredOutgoingWorkOrders = selectedOutgoingOrder
    ? outgoingWorkOrdersData.filter(
  ...
  useEffect(() => {
    if (
      activeTab === "outgoing" &&
      !selectedOutgoingOrder &&
      outgoingOrderNumbers.length > 0
    ) {
  ...
  useEffect(() => {
    if (
      ...
      if (currentWorkOrder) {
        const profiles =
          currentWorkOrder.profiles ||
          (currentWorkOrder.profile ? [currentWorkOrder.profile] : []);
        const cardIds = profiles.map(
          (_, index) => `${currentWorkOrder.id}-${index}`
        );
        const allSelected =
          cardIds.length > 0 && cardIds.every((id) => selectedCards.has(id));
      ...
      <div className="transaction-tabs">
        <button
          ...
          onClick={() => {
            ...
            navigate(
              `/dashboard/professional-consultancy/${category}/shortlisted?tab=incoming`,
              {
                replace: true,
              }
            );
        ...
        <button
          ...
          onClick={() => {
            ...
            navigate(
              `/dashboard/professional-consultancy/${category}/shortlisted?tab=outgoing`,
              {
                replace: true,
              }
            );
          ...
          {incomingWorkOrdersData?.length === 0 ? (
            ...
            <div className="shortlisted-cards-list">
              {incomingWorkOrdersData?.map((workOrder) => {
                const profiles =
                  workOrder.profiles ||
                  (workOrder.profile ? [workOrder.profile] : []);
                  ...
                  >
                    ...
                    {profiles?.map((profile, profileIndex) => {
                      ...
                      const isAccordionExpanded =
                        expandedAccordions.has(cardId);

                      return (
                        <div
                          key={cardId}
                          className={`shortlisted-card-full ${
                            isAccordionExpanded ? "accordion-expanded" : ""
                          }`}
                        >
                                ...
                                <div className="accordion-two-sections">
                                  ...
                                  <div className="accordion-price-form-section">
                                    ...
                                    <div className="price-form-item">
                                      ...
                                      <div
                                        className="collaboration-range-wrapper"
                                    ...
                                    <div className="price-form-item">
                                      ...
                                      <div className="offer-price-input-wrapper">
                                        ...
                                        >
                                          <option value="$500">
                                            $500 (24-10-2025 | 03:45 PM)
                                          </option>
                                          <option value="$1000">
                                            $1000 (24-10-2025 | 03:45 PM)
                                          </option>
                                          <option value="$1500">
                                            $1500 (24-10-2025 | 03:45 PM)
                                          </option>
                                          <option value="$2000">
                                            $2000 (24-10-2025 | 03:45 PM)
                                          </option>
                                          <option value="$2500">
                                            $2500 (24-10-2025 | 03:45 PM)
                                          </option>
                                          <option value="$3000">
                                    ...
                                    >
                                      <button
                                        ...
                                        onClick={() =>
                                          setShowAuctionInfluencerModal(true)
                                        }
                                      >
                                        Offer
                                      </button>
                                      <button
                                        ...
                                        onClick={() =>
                                          setShowAuctionCollaboratorModal(true)
                                        }
                                      >
                                        Accept
                                      </button>
                    ...
                    onChange={(e) => {
                      ...
                      if (isChecked) {
                        ...
                        const currentWorkOrder =
                          filteredOutgoingWorkOrders.find(
                            (wo) => wo.orderNumber === selectedOutgoingOrder
                          );
                        if (currentWorkOrder) {
                          const profiles =
                            currentWorkOrder.profiles ||
                            (currentWorkOrder.profile
                              ? [currentWorkOrder.profile]
                              : []);
                          const cardIds = profiles.map(
                            (_, index) => `${currentWorkOrder.id}-${index}`
...
export default ConsultancyCategoryShortlisted;
